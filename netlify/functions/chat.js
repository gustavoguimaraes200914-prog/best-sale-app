...
exports.handler = async (event) => {
  ...
  const nomeIdioma = NOMES_IDIOMA[idioma] || NOMES_IDIOMA['pt-BR'];
  const instrucaoIdioma = '\n\n=== REGRA DE IDIOMA (a mais importante deste prompt, tem prioridade sobre qualquer outra frase acima) ===\nResponda SEMPRE no mesmo idioma em que a pessoa escreveu a mensagem mais recente dela — identifique esse idioma sozinho, pelo próprio texto que ela mandou, sem precisar que ela peça. Se ela escrever em inglês, responda em inglês. Se escrever em espanhol, responda em espanhol. E assim por diante, para qualquer idioma, mesmo que ela troque de idioma no meio da conversa, e mesmo que todo o resto deste prompt esteja em português — essas instruções aqui são só o seu conhecimento e regras de comportamento, não definem em que idioma você fala com a pessoa. Vale pra tudo: texto corrido, títulos, tabelas, listas, anúncios gerados. Nomes próprios de plataformas (Mercado Livre, Shopee, Hotmart etc.) e valores em R$ continuam como estão, sem traduzir. Só use ' + nomeIdioma + ' como idioma padrão se a mensagem da pessoa não tiver texto suficiente pra identificar um idioma (ex: só números, um emoji, ou está vazia por causa de um anexo).';
  ...
  if (ultima && ultima.role === 'user') {
    const tag = '\n\n[Instrução de sistema — não faz parte do que a pessoa escreveu: identifique o idioma da mensagem acima e responda nesse mesmo idioma. Se não der pra identificar, use ' + nomeIdioma + '.]';
