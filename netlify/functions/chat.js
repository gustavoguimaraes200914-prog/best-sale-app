...
exports.handler = async (event) => {
  ...
  const instrucaoIdioma = '\n\n=== REGRA DE IDIOMA (a mais importante deste prompt, tem prioridade sobre qualquer outra frase acima) ===\nEscreva sua resposta INTEIRA em ' + nomeIdioma + '. Isso vale pra tudo: texto corrido, títulos, tabelas, listas, anúncios gerados, tudo. Não troque de idioma mesmo que o restante deste prompt tenha sido escrito em português — essas instruções são só o seu conhecimento e regras de comportamento, não definem em que idioma você fala com a pessoa. Só volte a escrever em outro idioma se a própria pessoa mudar de idioma na mensagem dela. Nomes próprios de plataformas (Mercado Livre, Shopee, Hotmart etc.) e valores em R$ continuam como estão, sem traduzir.';
  ...
  if (ultima && ultima.role === 'user') {
    const tag = '\n\n[Instrução de sistema — não faz parte do que a pessoa escreveu: responda em ' + nomeIdioma + ', sem exceção.]';
