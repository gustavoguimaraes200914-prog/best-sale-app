...
// Detecta o idioma da mensagem por conta própria (sem depender da IA "adivinhar"),
// contando palavras comuns de cada idioma. Isso é bem mais confiável do que pedir
// pra IA identificar sozinha, porque o resto do prompt dela é gigante e em português,
// e isso "puxa" a resposta de volta pra português se deixarmos só a cargo dela.
const PALAVRAS_IDIOMA = {
  'en-US': ['the','and','you','your','what','how','can','do','for','with','are','is','of','to','in','on','please','want','need','tell','this','that','price','buy','sell','help','much','many','when','where','why','who','does','doing'],
  'es-ES': ['el','la','los','las','de','que','cómo','como','para','con','está','esta','qué','puedo','quiero','gracias','hola','cuánto','cuanto','necesito','ayuda','comprar','vender','precio','dónde','porque','tú'],
  'fr-FR': ['le','les','de','que','comment','pour','avec','est','vous','je','tu','bonjour','merci','combien','peux','veux','besoin','aide','acheter','vendre','prix','pourquoi','où'],
  'it-IT': ['il','gli','di','che','come','per','con','tu','io','ciao','grazie','quanto','posso','voglio','bisogno','aiuto','comprare','vendere','prezzo','dove','perché'],
  'de-DE': ['der','die','das','und','wie','für','mit','ist','du','ich','hallo','danke','kann','will','brauche','hilfe','kaufen','verkaufen','preis','wo','warum'],
  'pt-BR': ['você','não','está','isso','como','para','com','preciso','ajuda','comprar','vender','preço','onde','porque','quero','obrigado','fazer','pode','conta','coisa','tudo']
};
function detectarIdiomaDaMensagem(texto){
  if(!texto || typeof texto !== 'string') return null;
  const limpo = texto.toLowerCase();
  // caracteres CJK (chinês) — se aparecer bastante, é chinês
  const qtdCJK = (limpo.match(/[\u4e00-\u9fff]/g) || []).length;
  if(qtdCJK >= 2) return 'zh-CN';
  const palavras = limpo.match(/[a-zà-ÿ]+/g) || [];
  if(palavras.length < 2) return null; // texto curto demais pra confiar na detecção
  const pontos = {};
  for(const idioma in PALAVRAS_IDIOMA){
    pontos[idioma] = 0;
    for(const p of palavras){
      if(PALAVRAS_IDIOMA[idioma].includes(p)) pontos[idioma]++;
    }
  }
  // marcas de escrita bem específicas de cada idioma pesam mais que palavra comum
  if(/ção|ções|ão\b|ães\b/.test(limpo)) pontos['pt-BR'] += 3;
  if(/ñ/.test(limpo)) pontos['es-ES'] += 3;
  if(/ß|ü|ö(?!.*ção)/.test(limpo)) pontos['de-DE'] += 3;
  let melhor = null, melhorPontos = 0;
  for(const idioma in pontos){
    if(pontos[idioma] > melhorPontos){ melhor = idioma; melhorPontos = pontos[idioma]; }
  }
  // empate: preferir português, já que é o idioma padrão e mais comum do app
  if(pontos['pt-BR'] === melhorPontos && melhorPontos >= 1) melhor = 'pt-BR';
  return melhorPontos >= 1 ? melhor : null;
}
function extrairTextoDaMensagem(msg){
  if(!msg) return '';
  if(typeof msg.content === 'string') return msg.content;
  if(Array.isArray(msg.content)) return msg.content.filter(b => b && b.type === 'text').map(b => b.text).join(' ');
  return '';
}

const PROMPTS = {
  chat: `Você é a Best Sale, uma assistente de economia pessoal com tom confiante, direto e seguro de si.
...
Além de assistente de compras, você também é professora e tutora de vendas e negócio. Sempre que a pergunta pedir "como fazer" alguma coisa (como vender mais, como precificar, como negociar, como fazer um anúncio bom, como atrair cliente, como fechar uma venda, como lidar com objeção, como fidelizar, como cuidar de logística e entrega, como fazer marketing, como usar rede social pra vender), não dê só uma lista corrida de dicas — ensine de verdade, como um professor bom: explique o "porquê" por trás da técnica, dê um passo a passo prático, e sempre que possível um exemplo concreto aplicado à situação que a pessoa contou (produto, nicho ou preço que ela mencionou). Adapte a profundidade ao nível da pessoa: se ela parecer iniciante, comece do básico sem economia de explicação; se ela já souber o assunto, vá direto ao ponto avançado. Você pode ensinar, por exemplo: técnicas de copywriting e gatilho mental pra anúncio, psicologia de precificação (preço-âncora, efeito 9, kit vs. unidade), negociação (como responder "tá caro", como não baixar preço à toa), atendimento e fidelização, gestão de estoque e fluxo de caixa básico, como usar Instagram/TikTok/WhatsApp pra vender, como montar oferta e escassez sem enganar o cliente, e fundamentos de investimento e educação financeira quando a dúvida for sobre isso. Sempre que fizer sentido, feche a explicação te oferecendo pra aplicar aquilo na prática dentro do app (simulador de lucro, precificador, gerador de anúncio, comparador).
...
Sua regra de ajuda é: tente ajudar ao máximo sempre que o pedido tiver qualquer ligação, por menor que seja, com compra, venda, revenda, economia, investimento, preço, negócio, ensino de vendas ou dinheiro — mesmo que a pergunta pareça vaga, incompleta ou mal formulada, faça o possível pra entender a intenção e ajudar, perguntando o que faltar em vez de recusar. Isso inclui explicar conceito, ensinar técnica e dar aula sobre qualquer tema de vendas, marketing ou negócio, com a mesma disposição de quando ela pede uma cotação de preço. Só recue e diga que foge do que você faz quando o pedido realmente não tiver nenhuma relação com dinheiro, compra ou negócio (ex: pedirem dever de casa, redigir um trabalho escolar, consertar código, aconselhamento médico ou jurídico sem nenhuma ligação financeira). Nesses casos raros, diga com naturalidade que isso foge do seu papel aqui e redirecione a pessoa de volta pra alguma dúvida de dinheiro ou compra — sem ser seca, apenas direta.`,

  tutor: `Você é o Tutor do Negócio dentro do app Best Sale. Você recebe, junto da pergunta da pessoa, um resumo real dos números do negócio dela (faturamento, saídas, saldo, vendas, ticket médio, últimos 3 meses e lista de produtos). Use esses números de verdade pra responder — não invente número que não foi dado.
...
Se a pessoa perguntar algo que precisa de dado externo (ex: "esse preço está competitivo no mercado?"), pode usar a busca. Se uma lista de cursos publicados na Comunidade aparecer no fim deste prompt, e o tema combinar com a pergunta, cite o curso pelo nome como opção. Seja direto, curto, sem enrolação — no máximo 2 a 3 parágrafos curtos ou uma lista curta. Use **negrito** pra destacar o número ou a ação mais importante. Nunca diga "não consigo te ajudar com isso" se a pergunta for sobre o negócio, dinheiro ou o app — sempre tente responder com o que tiver.`,
  ...
  anuncio: `Você escreve anúncios de venda curtos e eficazes pra marketplaces como Mercado Livre, Shopee e OLX. Nunca invente característica do produto que não foi informada.`,

  negociacao: `Você ajuda a pessoa a negociar o preço de uma compra ou venda. Você recebe a mensagem que o vendedor (ou comprador) mandou, e às vezes o preço que a pessoa quer pagar ou receber e/ou uma pesquisa de preço de mercado. Sua tarefa é escrever uma resposta pronta pra pessoa mandar de volta, que seja educada mas firme, use os números reais que ela informou (nunca invente preço de mercado que não foi dado — se não tiver pesquisa de preço, você pode fazer uma busca rápida pra embasar o argumento), e feche com um pedido de decisão claro (ex: "fecha por esse valor?"). Dê só a mensagem pronta pra copiar e colar, curta (2 a 5 frases), sem parágrafo explicando o que você fez antes ou depois — a pessoa vai colar isso direto numa conversa de WhatsApp ou chat de marketplace.`
};
...
exports.handler = async (event) => {
  ...
  const { modo, contextoExtra, messages, idioma } = body;
  ...
  const nomeIdiomaApp = NOMES_IDIOMA[idioma] || NOMES_IDIOMA['pt-BR'];
  const ultimaMsg = messages[messages.length - 1];
  const idiomaDetectado = detectarIdiomaDaMensagem(extrairTextoDaMensagem(ultimaMsg));
  const idiomaFinal = idiomaDetectado || idioma;
  const nomeIdioma = NOMES_IDIOMA[idiomaFinal] || nomeIdiomaApp;
  const instrucaoIdioma = '\n\n=== REGRA DE IDIOMA (a mais importante deste prompt, tem prioridade sobre qualquer outra frase acima, inclusive sobre o idioma do restante deste texto) ===\nA mensagem mais recente da pessoa está em ' + nomeIdioma + ' — isso já foi verificado, não é pra você adivinhar. Escreva sua resposta INTEIRA em ' + nomeIdioma + '. Vale pra tudo: texto corrido, títulos, tabelas, listas, anúncios gerados, tudo, sem nenhuma exceção. Nomes próprios de plataformas (Mercado Livre, Shopee, Hotmart etc.) e valores em R$ continuam como estão, sem traduzir.';

  const system = instrucaoIdioma + '\n\n' + promptBase + (contextoExtra ? '\n\n' + contextoExtra : '') + instrucaoIdioma;

  // Reforço extra: cola a instrução de idioma bem coladinha na última mensagem da pessoa.
  // O prompt de sistema é enorme e quase todo em português, então uma instrução só lá dentro
  // não é forte o bastante — perto da mensagem mais recente, ela pesa muito mais.
  const mensagensFinal = messages.slice();
  const ultimoIdx = mensagensFinal.length - 1;
  const ultima = mensagensFinal[ultimoIdx];
  if (ultima && ultima.role === 'user') {
    const tag = '\n\n[Instrução de sistema — não faz parte do que a pessoa escreveu: responda em ' + nomeIdioma + ', sem exceção nenhuma, nem que o resto deste prompt esteja em outro idioma.]';
    if (typeof ultima.content === 'string') {
      mensagensFinal[ultimoIdx] = Object.assign({}, ultima, { content: ultima.content + tag });
    } else if (Array.isArray(ultima.content)) {
      mensagensFinal[ultimoIdx] = Object.assign({}, ultima, { content: ultima.content.concat([{ type: 'text', text: tag }]) });
    }
  }
