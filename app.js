...
(function(){
  ...
  const CHAVE_GARANTIAS = 'bestsale:garantias';
  ...
  function precisaLogin(){ return !!auth && !auth.currentUser; }
  document.querySelectorAll('[data-fecha]').forEach(b => b.addEventListener('click', () => {
    if(b.dataset.fecha === 'overlay-perfil' && precisaLogin()) return;
    fechar(b.dataset.fecha);
  }));
  document.querySelectorAll('.overlay').forEach(o => o.addEventListener('click', ev => {
    if(ev.target !== o) return;
    if(o.id === 'overlay-perfil' && precisaLogin()) return;
  ...
  // ===== Analisar mensagem em blocos estruturados (usado pra exportar em PDF/Word/Excel/PPT) =====
  function prepararTextoPlano(texto){
    return texto.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g, '$1 ($2)');
  }
  function segmentosNegrito(texto){
    const limpo = prepararTextoPlano(texto);
    const partes = [];
    const regex = /\*\*([^*]+)\*\*/g;
    let ultimo = 0, m;
    while((m = regex.exec(limpo))){
      if(m.index > ultimo) partes.push({ texto: limpo.slice(ultimo, m.index), negrito:false });
      partes.push({ texto: m[1], negrito:true });
      ultimo = regex.lastIndex;
    }
    if(ultimo < limpo.length) partes.push({ texto: limpo.slice(ultimo), negrito:false });
    return partes.map(p => ({ ...p, texto: p.texto.replace(/\*/g,'') })).filter(p => p.texto !== '');
  }
  function analisarBlocosMensagem(bruto){
    const blocos = [];
    const graficosDados = [];
    const base = bruto.replace(/```grafico\n([\s\S]*?)```/g, (m,c) => {
      const linhasG = c.split('\n').map(l=>l.trim()).filter(Boolean);
      let titulo=''; const itens=[];
      linhasG.forEach(l => {
        const tt = /^t[íi]tulo:\s*(.+)/i.exec(l);
        if(tt){ titulo = tt[1]; return; }
        const it = /^(.+?):\s*(-?\d+(?:[.,]\d+)?)\s*$/.exec(l);
        if(it) itens.push({ rotulo: it[1].trim(), valor: parseFloat(it[2].replace(',','.')) });
      });
      graficosDados.push({ titulo, itens });
      return '\u0001'+(graficosDados.length-1)+'\u0001';
    });
    const linhas = base.split('\n');
    let lista = null, itensLista = [], par = [];
    const fechaPar = () => { if(par.length){ blocos.push({ tipo:'p', partes: segmentosNegrito(par.join(' ')) }); par = []; } };
    const fechaLista = () => { if(lista){ blocos.push({ tipo:'lista', ordenado: lista==='ol', itens: itensLista }); itensLista = []; lista = null; } };
    let i = 0;
    while(i < linhas.length){
      const l = linhas[i].trim();
      const mg = /^\u0001(\d+)\u0001$/.exec(l);
      if(mg){ fechaPar(); fechaLista(); blocos.push({ tipo:'grafico', ...graficosDados[Number(mg[1])] }); i++; continue; }
      const ehTab = /^\|.*\|$/.test(l);
      const sep = linhas[i+1] && /^\|?\s*:?-{2,}:?\s*(\|\s*:?-{2,}:?\s*)+\|?$/.test(linhas[i+1].trim());
      if(ehTab && sep){
        fechaPar(); fechaLista();
        const cel = s => s.trim().split('|').map(c=>c.trim()).filter((c,idxC,arr)=> !(idxC===0&&c==='') && !(idxC===arr.length-1&&c===''));
        const cab = cel(l); let j = i+2; const corpo = [];
        while(j < linhas.length && /^\|.*\|$/.test(linhas[j].trim())){ corpo.push(cel(linhas[j])); j++; }
        blocos.push({ tipo:'tabela', cabecalho: cab.map(prepararTextoPlano), linhas: corpo.map(r=>r.map(prepararTextoPlano)) });
        i = j; continue;
      }
      const ul = /^[-•]\s+(.*)/.exec(l);
      const ol = /^\d+[.)]\s+(.*)/.exec(l);
      if(ul){ fechaPar(); if(lista!=='ul'){ fechaLista(); lista='ul'; } itensLista.push(segmentosNegrito(ul[1])); }
      else if(ol){ fechaPar(); if(lista!=='ol'){ fechaLista(); lista='ol'; } itensLista.push(segmentosNegrito(ol[1])); }
      else if(l === ''){ fechaPar(); fechaLista(); }
      else { fechaLista(); par.push(l); }
      i++;
    }
    fechaPar(); fechaLista();
    return blocos;
  }
  function tituloArquivoMensagem(bruto){
    const primeiraLinha = bruto.split('\n').map(l=>l.trim()).find(l => l.length > 0) || 'best-sale';
    return primeiraLinha.replace(/[*_#`]/g,'').replace(/[^\p{L}\p{N}\s-]/gu,'').trim().slice(0,60).replace(/\s+/g,'-').toLowerCase() || 'best-sale';
  }
  async function compartilharTexto(titulo, texto){
    if(navigator.share){
      try{ await navigator.share({ title: titulo, text: texto }); }catch(e){ /* pessoa cancelou o compartilhamento, tudo bem */ }
    } else if(navigator.clipboard && navigator.clipboard.writeText){
      try{ await navigator.clipboard.writeText(texto); toast(t('toast_copiado_area')); }
      catch(e){ toast(texto); }
    } else {
      toast(texto);
    }
  ...
  function exportarComoHTML(bruto){
    const corpo = formatarMensagem(bruto);
    const html = '<!DOCTYPE html><html lang="'+idiomaAtual+'"><head><meta charset="UTF-8"><title>Best Sale</title>'
      + '<style>body{background:#12100B;color:#F4EAD2;font-family:Georgia,serif;max-width:720px;margin:40px auto;padding:0 20px;line-height:1.7} '
      + 'h1{color:#FFBE00;font-size:1.4rem;border-bottom:1px solid #4A3F26;padding-bottom:12px} strong{color:#FFBE00} '
      + 'table{width:100%;border-collapse:collapse;margin:16px 0} th,td{border:1px solid #4A3F26;padding:8px 10px;text-align:left} th{background:#1E1A12} '
      + 'a{color:#FFBE00} ul,ol{padding-left:22px} .grafico-wrap{margin:16px 0} .linha-grafico{display:flex;align-items:center;gap:10px;margin:6px 0} '
      + '.rotulo-grafico{width:120px;font-size:.85rem;color:#C9BFA3} .trilha-grafico{flex:1;background:#241F16;border-radius:6px;height:14px;overflow:hidden} '
      + '.barra-grafico{background:#FFBE00;height:100%} .valor-grafico{font-size:.85rem;min-width:60px;text-align:right}</style></head>'
      + '<body><h1>Best Sale</h1>' + corpo + '</body></html>';
    const blob = new Blob([html], { type:'text/html;charset=utf-8' });
    baixarArquivo(blob, tituloArquivoMensagem(bruto) + '.html');
  }

  function exportarComoPDF(bruto){
    ...
    let y = margem;
    function quebrarSeNecessario(altura){ if(y + altura > alturaPagina - margem){ doc.addPage(); y = margem; } }
    function escreverPartes(partes, tamanho, xInicial, larguraMax){
      doc.setFontSize(tamanho);
      const linhaAltura = tamanho * 1.5;
      let x = xInicial;
      quebrarSeNecessario(linhaAltura);
      doc.setFont('helvetica','normal');
      const espaco = doc.getTextWidth(' ');
      partes.forEach(p => {
        p.texto.split(/\s+/).filter(Boolean).forEach(palavra => {
          doc.setFont('helvetica', p.negrito ? 'bold' : 'normal');
          const largura = doc.getTextWidth(palavra);
          if(x + largura > xInicial + larguraMax){ x = xInicial; y += linhaAltura; quebrarSeNecessario(linhaAltura); }
    ...
    doc.text('Best Sale', margem, y); y += 26;
    doc.setDrawColor(200,200,200); doc.line(margem, y, larguraPagina-margem, y); y += 18;
    doc.setTextColor(20,20,20);
    const blocos = analisarBlocosMensagem(bruto);
    blocos.forEach(b => {
      if(b.tipo === 'p'){ escreverPartes(b.partes, 11, margem, larguraUtil); }
      else if(b.tipo === 'lista'){
        b.itens.forEach((partes, idxItem) => {
          const prefixo = b.ordenado ? (idxItem+1)+'. ' : '•  ';
          quebrarSeNecessario(16);
          doc.setFont('helvetica','normal'); doc.setFontSize(11);
          doc.text(prefixo, margem, y);
          const larguraPrefixo = doc.getTextWidth(prefixo) + 2;
          escreverPartes(partes, 11, margem + larguraPrefixo, larguraUtil - larguraPrefixo);
        });
      }
      else if(b.tipo === 'tabela'){
        const colunas = b.cabecalho.length || 1;
        const larguraCol = larguraUtil / colunas;
        quebrarSeNecessario(20);
        doc.setFont('helvetica','bold'); doc.setFontSize(10);
        b.cabecalho.forEach((c,idxC) => doc.text(String(c).slice(0,40), margem + idxC*larguraCol, y));
        y += 6; doc.setDrawColor(180,180,180); doc.line(margem, y, larguraPagina-margem, y); y += 14;
        doc.setFont('helvetica','normal');
        b.linhas.forEach(linha => {
          quebrarSeNecessario(16);
          linha.forEach((c,idxC) => doc.text(String(c).slice(0,40), margem + idxC*larguraCol, y));
          y += 16;
        });
        y += 10;
      }
        ...
        quebrarSeNecessario(20);
        if(b.titulo){ doc.setFont('helvetica','bold'); doc.setFontSize(11); doc.text(b.titulo, margem, y); y += 18; }
        const max = Math.max.apply(null, (b.itens||[]).map(it=>Math.abs(it.valor)).concat([1]));
        (b.itens||[]).forEach(it => {
          quebrarSeNecessario(18);
          doc.setFont('helvetica','normal'); doc.setFontSize(10);
          doc.text(String(it.rotulo).slice(0,25), margem, y);
          const xBarra = margem + 110, larguraMaxBarra = larguraUtil - 170;
          const larguraBarra = Math.max(3, (Math.abs(it.valor)/max) * larguraMaxBarra);
          doc.setFillColor(255,190,0); doc.rect(xBarra, y-8, larguraBarra, 10, 'F');
          doc.text(String(it.valor), xBarra + larguraMaxBarra + 8, y);
          y += 16;
        });
        y += 10;
      }
    });
    doc.save(tituloArquivoMensagem(bruto) + '.pdf');
  }

  async function exportarComoWord(bruto){
    const docx = window.docx;
    const blocos = analisarBlocosMensagem(bruto);
    const filhos = [ new docx.Paragraph({ text:'Best Sale', heading: docx.HeadingLevel.HEADING_1 }) ];
    const paraRuns = partes => partes.map(p => new docx.TextRun({ text:p.texto, bold: !!p.negrito }));
    blocos.forEach(b => {
      if(b.tipo === 'p'){ filhos.push(new docx.Paragraph({ children: paraRuns(b.partes), spacing:{ after:160 } })); }
      else if(b.tipo === 'lista'){
        b.itens.forEach(partes => filhos.push(new docx.Paragraph({ children: paraRuns(partes), bullet:{ level:0 }, spacing:{ after:80 } })));
      }
      else if(b.tipo === 'tabela'){
        const linhaTabela = (celulas, cabecalho) => new docx.TableRow({ children: celulas.map(c => new docx.TableCell({
          children:[ new docx.Paragraph({ children:[ new docx.TextRun({ text:String(c), bold: !!cabecalho }) ] }) ]
        })) });
        const tabela = new docx.Table({ rows:[ linhaTabela(b.cabecalho, true), ...b.linhas.map(l=>linhaTabela(l,false)) ], width:{ size:100, type: docx.WidthType.PERCENTAGE } });
        filhos.push(tabela);
        filhos.push(new docx.Paragraph({ text:'', spacing:{ after:160 } }));
      }
      else if(b.tipo === 'grafico'){
        if(b.titulo) filhos.push(new docx.Paragraph({ children:[ new docx.TextRun({ text:b.titulo, bold:true }) ], spacing:{ after:80 } }));
        (b.itens||[]).forEach(it => filhos.push(new docx.Paragraph({ text: it.rotulo + ':  ' + it.valor, spacing:{ after:40 } })));
        filhos.push(new docx.Paragraph({ text:'', spacing:{ after:120 } }));
      }
    });
    const documento = new docx.Document({ sections:[{ children: filhos }] });
    const blob = await docx.Packer.toBlob(documento);
    baixarArquivo(blob, tituloArquivoMensagem(bruto) + '.docx');
  }

  function exportarComoExcel(bruto){
    const blocos = analisarBlocosMensagem(bruto);
    const tabelas = blocos.filter(b => b.tipo === 'tabela');
    const wb = XLSX.utils.book_new();
    if(tabelas.length){
      tabelas.forEach((tb, idxT) => {
        const dados = [ tb.cabecalho, ...tb.linhas ];
        const ws = XLSX.utils.aoa_to_sheet(dados);
        XLSX.utils.book_append_sheet(wb, ws, 'Tabela'+(idxT+1));
      });
    }
    const graficos = blocos.filter(b => b.tipo === 'grafico');
    graficos.forEach((g, idxG) => {
      const dados = [ ['Item','Valor'], ...(g.itens||[]).map(it => [it.rotulo, it.valor]) ];
      const ws = XLSX.utils.aoa_to_sheet(dados);
      XLSX.utils.book_append_sheet(wb, ws, (g.titulo || 'Grafico'+(idxG+1)).slice(0,28));
    });
    if(!tabelas.length && !graficos.length){
      const linhasTexto = [];
      blocos.forEach(b => {
        if(b.tipo === 'p') linhasTexto.push([ b.partes.map(p=>p.texto).join('') ]);
        else if(b.tipo === 'lista') b.itens.forEach(it => linhasTexto.push([ '- ' + it.map(p=>p.texto).join('') ]));
      });
      const ws = XLSX.utils.aoa_to_sheet([['Best Sale'], [''], ...linhasTexto]);
      XLSX.utils.book_append_sheet(wb, ws, 'Resposta');
    }
    XLSX.writeFile(wb, tituloArquivoMensagem(bruto) + '.xlsx');
  }

  function exportarComoPPT(bruto){
    const blocos = analisarBlocosMensagem(bruto);
    const pres = new window.PptxGenJS();
    const slideCapa = pres.addSlide();
    slideCapa.background = { color:'12100B' };
    slideCapa.addText('Best Sale', { x:0.5, y:2.2, w:9, h:1, fontSize:36, bold:true, color:'FFBE00', align:'center' });
    let slideAtual = pres.addSlide();
    slideAtual.background = { color:'12100B' };
    let yAtual = 0.4;
    const novoSlideSeNecessario = altura => {
      if(yAtual + altura > 6.8){ slideAtual = pres.addSlide(); slideAtual.background = { color:'12100B' }; yAtual = 0.4; }
    };
    blocos.forEach(b => {
      if(b.tipo === 'p'){
        const texto = b.partes.map(p=>p.texto).join('');
        const linhas = Math.ceil(texto.length / 90) || 1;
        const altura = 0.3 * linhas + 0.15;
        novoSlideSeNecessario(altura);
        slideAtual.addText(b.partes.map(p=>({ text:p.texto, options:{ bold: !!p.negrito, color:'F4EAD2', fontSize:14 } })), { x:0.5, y:yAtual, w:9, h:altura });
        yAtual += altura + 0.1;
      } else if(b.tipo === 'lista'){
        const linhasTxt = b.itens.map(it => ({ text: it.map(p=>p.texto).join(''), options:{ bullet:true, color:'F4EAD2', fontSize:14 } }));
        const altura = 0.35 * linhasTxt.length + 0.15;
        novoSlideSeNecessario(altura);
        slideAtual.addText(linhasTxt, { x:0.5, y:yAtual, w:9, h:altura });
        yAtual += altura + 0.1;
      } else if(b.tipo === 'tabela'){
        const altura = 0.35 * (b.linhas.length+1) + 0.2;
        novoSlideSeNecessario(Math.min(altura, 6));
        const linhasTabela = [ b.cabecalho.map(c => ({ text:String(c), options:{ bold:true, color:'12100B', fill:{ color:'FFBE00' } } })), ...b.linhas.map(l => l.map(c => ({ text:String(c), options:{ color:'F4EAD2' } }))) ];
        slideAtual.addTable(linhasTabela, { x:0.5, y:yAtual, w:9, fontSize:11, border:{ color:'4A3F26', pt:1 } });
        yAtual += Math.min(altura, 6) + 0.15;
      } else if(b.tipo === 'grafico'){
        novoSlideSeNecessario(3.5);
        if((b.itens||[]).length){
          slideAtual.addChart(pres.ChartType.bar, [{ name: b.titulo || 'Dados', labels: b.itens.map(it=>String(it.rotulo)), values: b.itens.map(it=>it.valor) }], { x:0.5, y:yAtual, w:9, h:3.2, chartColors:['FFBE00'], catAxisLabelColor:'F4EAD2', valAxisLabelColor:'F4EAD2' });
          yAtual += 3.4;
        }
      }
    });
    pres.writeFile({ fileName: tituloArquivoMensagem(bruto) + '.pptx' });
  }



  const ABERTURAS = [
    'Fala, {nome}.', 'Bora, {nome}?', 'Que bom te ver, {nome}.', 'Pronto pra hoje, {nome}?',
    'No que vamos mexer hoje, {nome}?', '{nome}, por onde começamos?', 'Trabalhando até tarde, {nome}?',
    'De volta, {nome}.', 'Vamos ver esse dinheiro, {nome}?', 'Por onde começamos?'
  ];
  ...
  function bolha(m, idx){
    ...
    const botaoExportar = '<button class="btn-exportar-msg" data-idx-exportar="'+idx+'"><span>⬇</span><span>'+esc(t('btn_baixar_msg'))+'</span></button>';
    return '<div class="mensagem-assistente"><div class="rotulo-assistente"><span class="bola"></span>Best Sale</div>'+formatarMensagem(m.texto)+botaoExportar+'</div>';
  }
  ...
  function escolherAbertura(){
    ...
    const nome = (perfil.nome || '').trim().split(' ')[0];
    const disponiveis = nome ? lista : lista.filter(f => !f.includes('{nome}'));
    const escolhida = disponiveis[Math.floor(Math.random()*disponiveis.length)];
    return nome ? escolhida.replace(/\{nome\}/g, nome) : escolhida;
  }
  ...
  function renderizarChat(base, parcial){
    ...
    if(!lista.length && !carregando && parcial === undefined){
      c.innerHTML = '<div class="vazio-chat"><div class="vazio-icone">'+SACOLA_MINI+'</div><div class="vazio-titulo">'+esc(aberturaEscolhida)+'</div></div>';
      return;
    }
  ...
  async function chamarClaudeGenerico(modo, contextoExtra, listaMensagens){
    ...
    try{
      resposta = await fetch('/.netlify/functions/chat', {
        ...
        body: JSON.stringify({ modo, contextoExtra: contextoExtra || '', messages: paraAPI, idioma: idiomaAtual })
      });
    }catch(e){
  ...
  $('pergunta-tutor').addEventListener('keydown', ev => { if(ev.key==='Enter' && !ev.shiftKey){ ev.preventDefault(); perguntarTutor(); } });
  ...
  // ===== Exportar mensagem em PDF/Word/Excel/PowerPoint/HTML =====
  let textoParaExportar = '';
  $('mensagens').addEventListener('click', ev => {
    const btn = ev.target.closest('.btn-exportar-msg');
    if(!btn) return;
    const idx = Number(btn.dataset.idxExportar);
    const msg = mensagens[idx];
    if(!msg) return;
    textoParaExportar = msg.texto;
    abrir('overlay-exportar-msg');
  });
  function comFeedbackErro(fn){
    return () => {
      try{
        const resultado = fn(textoParaExportar);
        if(resultado && typeof resultado.catch === 'function') resultado.catch(e => { console.error(e); toast(t('erro_exportar')); });
        fechar('overlay-exportar-msg');
      }
      catch(e){ console.error(e); toast(t('erro_exportar')); }
    };
  }
  $('exp-pdf').addEventListener('click', comFeedbackErro(exportarComoPDF));
  $('exp-word').addEventListener('click', comFeedbackErro(exportarComoWord));
  $('exp-excel').addEventListener('click', comFeedbackErro(exportarComoExcel));
  $('exp-ppt').addEventListener('click', comFeedbackErro(exportarComoPPT));
  $('exp-html').addEventListener('click', comFeedbackErro(exportarComoHTML));

  function ajustarAlturaAuto(campo){
    campo.style.height = 'auto';
    campo.style.height = campo.scrollHeight + 'px';
  }
  document.querySelectorAll('.campo-auto-crescer').forEach(campo => {
    campo.addEventListener('input', () => ajustarAlturaAuto(campo));
  });

  $('btn-enviar').addEventListener('click', enviar);
  $('input-barra').addEventListener('keydown', ev => { if(ev.key === 'Enter' && !ev.shiftKey){ ev.preventDefault(); enviar(); } });
  ...
  function calcularResumoSemana(){
    const agora = Date.now();
    const semDia = 24*60*60*1000;
    const inicioAtual = agora - 7*semDia;
    const inicioAnterior = agora - 14*semDia;
    const atual = lancamentos.filter(l => l.criado >= inicioAtual);
    const anterior = lancamentos.filter(l => l.criado >= inicioAnterior && l.criado < inicioAtual);
    if(!atual.length) return null;
    const somar = (lista, tipo) => lista.filter(l=>l.tipo===tipo).reduce((s,l)=>s+l.valor,0);
    const receitaAtual = somar(atual,'receita');
    const despesaAtual = somar(atual,'despesa');
    const receitaAnterior = somar(anterior,'receita');
    const vendasAtual = atual.filter(l=>l.tipo==='receita' && l.venda).reduce((s,l)=>s+(l.qtd||1),0);
    let variacao = null;
    if(receitaAnterior > 0) variacao = ((receitaAtual - receitaAnterior) / receitaAnterior) * 100;
    return { receitaAtual, despesaAtual, saldoAtual: receitaAtual-despesaAtual, vendasAtual, variacao };
  }

  function renderizarResumoSemana(){
    const area = $('resumo-semana');
    const r = calcularResumoSemana();
    if(!r){ area.innerHTML = ''; return; }
    let linhaVariacao = '';
    if(r.variacao !== null){
      const seta = r.variacao >= 0 ? '📈' : '📉';
      linhaVariacao = '<div class="item-resumo" style="margin-top:8px"><span>'+t('txt_vs_semana_passada')+'</span><strong>'+seta+' '+(r.variacao>=0?'+':'')+r.variacao.toFixed(0)+'%</strong></div>';
    }
    area.innerHTML = '<div class="bloco">'
      + '<div class="titulo-bloco"><h3>'+esc(t('h3_resumo_semana'))+'</h3></div>'
      + '<div class="item-resumo"><span>'+esc(t('txt_faturou_semana'))+'</span><strong>'+moeda(r.receitaAtual)+'</strong></div>'
      + '<div class="item-resumo" style="margin-top:8px"><span>'+esc(t('txt_vendas_min')).replace(/^./, c=>c.toUpperCase())+'</span><strong>'+r.vendasAtual+'</strong></div>'
      + '<div class="item-resumo" style="margin-top:8px"><span>'+esc(t('card_saldo'))+'</span><strong>'+moeda(r.saldoAtual)+'</strong></div>'
      + linhaVariacao
      + '</div>';
  }
  ...
  function renderizarMetas(){
    ...
    area.innerHTML = metas.map(m => {
      ...
      return '<div class="item-produto">'
        ...
        + '<div class="barra-progresso"><div style="width:'+pct+'%"></div></div>'
        + '</div>'
        + '<div class="acoes-produto"><button class="btn-mini" data-deposito="'+m.id+'">'+t('btn_mais_guardar')+'</button><button class="btn-mini" data-compartilhar-meta="'+m.id+'">'+t('btn_compartilhar')+'</button><button class="btn-mini perigo" data-del-meta="'+m.id+'">'+t('btn_excluir')+'</button></div>'
        + '</div>';
    }).join('');
    ...
    area.querySelectorAll('[data-compartilhar-meta]').forEach(b => b.addEventListener('click', () => {
      const m = metas.find(x=>x.id===b.dataset.compartilharMeta);
      if(!m) return;
      const pct = Math.min(100, Math.round((m.acumulado / m.valorAlvo)*100));
      const texto = t('txt_compartilhar_meta_1').replace('{nome}', m.nome) + ' ' + moeda(m.acumulado) + ' ' + t('txt_de') + ' ' + moeda(m.valorAlvo) + ' (' + pct + '%). ' + t('txt_compartilhar_meta_2');
      compartilharTexto(m.nome, texto);
    }));
  ...
  // ===== Modo negociação =====
  $('btn-abrir-negociacao').addEventListener('click', () => {
    $('negociacao-mensagem').value=''; $('negociacao-alvo').value='';
    $('resultado-negociacao').innerHTML = '';
    abrir('overlay-negociacao');
  });
  $('btn-gerar-negociacao').addEventListener('click', async () => {
    const papel = $('negociacao-papel').value;
    const mensagem = $('negociacao-mensagem').value.trim();
    const alvo = parseFloat($('negociacao-alvo').value) || 0;
    if(!mensagem){ toast(t('toast_cole_mensagem')); return; }
    const area = $('resultado-negociacao');
    $('btn-gerar-negociacao').disabled = true;
    area.innerHTML = '<div class="carregando" style="padding-left:0"><span class="sacola-carregando">'+SACOLA_MINI+'</span><span class="texto-pensando">pensando</span></div>';
    const promptNegociacao = 'Eu estou '+(papel==='comprando'?'comprando':'vendendo')+' um produto. A outra pessoa me mandou esta mensagem: "'+mensagem+'".'+(alvo>0?' Eu quero '+(papel==='comprando'?'pagar':'receber')+' '+moeda(alvo)+'.':' Não pesquisei um valor específico ainda — se puder, pesquise um preço de mercado justo pra embasar minha resposta.')+' Escreva a resposta que eu devo mandar de volta.';
    let resp;
    try{ resp = await chamarClaudeGenerico('negociacao', '', [{ role:'user', texto: promptNegociacao }]); }
    catch(e){ resp = e && e.message ? e.message : t('erro_tente_novo_instantes'); }
    $('btn-gerar-negociacao').disabled = false;
    area.innerHTML = '<div class="mensagem-assistente" style="padding-left:0;border-left:none">'+formatarMensagem(resp)+'</div>';
  });
  ...
  async function verificarAlertaPreco(a){
  ...
  function renderizarAlertasPreco(){
    ...
    area.querySelectorAll('[data-verificar]').forEach(b => b.addEventListener('click', async () => {
      ...
      const resp = await verificarAlertaPreco(a);
      await Guardar.gravar(CHAVE_ALERTAS_PRECO, alertasPreco);
      renderizarAlertasPreco();
  ...
  // Checagem automática: quando o app abre, se já fazem 24h+ desde a última vez, verifica sozinho (silenciosamente) sem precisar tocar em "Verificar agora".
  async function verificarAlertasVencidosAutomaticamente(){
    const UM_DIA = 24*60*60*1000;
    const vencidos = alertasPreco.filter(a => !a.ultimaChecagem || (Date.now() - a.ultimaChecagem) > UM_DIA);
    if(!vencidos.length) return;
    for(const a of vencidos.slice(0, 3)){ // limite de 3 por abertura, pra não estourar a cota da API de uma vez
      await verificarAlertaPreco(a);
    }
    await Guardar.gravar(CHAVE_ALERTAS_PRECO, alertasPreco);
    if(telaAtual === 'dashboard') renderizarAlertasPreco();
    toast(t('toast_alertas_verificados').replace('{n}', String(Math.min(vencidos.length,3))), 6000);
  }
  ...
  function renderizarGarantias(){
    const area = $('lista-garantias');
    if(!garantias.length){ area.innerHTML = '<div class="vazio-bloco">'+t('vazio_garantias')+'</div>'; return; }
    const hoje = new Date(); hoje.setHours(0,0,0,0);
    const ordenadas = garantias.slice().sort((a,b) => new Date(a.vencimento) - new Date(b.vencimento));
    area.innerHTML = ordenadas.map(g => {
      const venc = new Date(g.vencimento + 'T00:00:00');
      const diasRestantes = Math.round((venc - hoje) / 86400000);
      let cor = '', situacao;
      if(diasRestantes < 0){ cor='negativo'; situacao = t('txt_garantia_vencida'); }
      else if(diasRestantes <= 15){ cor='negativo'; situacao = t('txt_garantia_vence_em') + ' ' + diasRestantes + ' ' + t('txt_dias'); }
      else { situacao = t('txt_garantia_vence_em') + ' ' + diasRestantes + ' ' + t('txt_dias'); }
      const capa = g.foto
        ? '<div class="capa-produto" style="background-image:url(data:'+g.foto.mediaType+';base64,'+g.foto.dataBase64+');background-size:cover;background-position:center"></div>'
        : '<div class="capa-produto" style="background:'+corCapa(g.id)+'">🧾</div>';
      return '<div class="item-produto">'+capa
        + '<div class="info-produto"><div class="nome-produto">'+esc(g.nome)+'</div>'
        + '<div class="meta-produto">'+t('txt_comprado_em')+' '+new Date(g.dataCompra+'T00:00:00').toLocaleDateString(idiomaAtual)+'</div>'
        + '<div class="meta-produto" style="color:var(--'+(cor==='negativo'?'coral':'papel-suave')+')">'+situacao+'</div></div>'
        + '<div class="acoes-produto"><button class="btn-mini perigo" data-del-garantia="'+g.id+'">'+t('btn_excluir')+'</button></div></div>';
    }).join('');
    area.querySelectorAll('[data-del-garantia]').forEach(b => b.addEventListener('click', async () => {
      garantias = garantias.filter(x=>x.id!==b.dataset.delGarantia);
      await Guardar.gravar(CHAVE_GARANTIAS, garantias);
      renderizarGarantias();
    }));
  }
  $('btn-nova-garantia').addEventListener('click', () => {
    $('garantia-nome').value=''; $('garantia-data-compra').value=''; $('garantia-dias').value='90'; $('garantia-foto').value='';
    abrir('overlay-garantia');
  });
  $('btn-salvar-garantia').addEventListener('click', async () => {
    const nome = $('garantia-nome').value.trim();
    const dataCompra = $('garantia-data-compra').value;
    const dias = parseInt($('garantia-dias').value, 10) || 0;
    if(!nome || !dataCompra || dias<=0){ toast(t('toast_preencha_garantia')); return; }
    const vencimentoData = new Date(dataCompra + 'T00:00:00');
    vencimentoData.setDate(vencimentoData.getDate() + dias);
    const vencimento = vencimentoData.getFullYear()+'-'+String(vencimentoData.getMonth()+1).padStart(2,'0')+'-'+String(vencimentoData.getDate()).padStart(2,'0');
    let foto = null;
    const arq = $('garantia-foto').files[0];
    if(arq){
      if(arq.size > MAX){ toast(t('toast_foto_muito_grande')); return; }
      try{ foto = { mediaType: arq.type, dataBase64: await base64De(arq) }; }catch(e){}
    }
    garantias.push({ id:idNovo(), nome, dataCompra, dias, vencimento, foto, criado:Date.now() });
    await Guardar.gravar(CHAVE_GARANTIAS, garantias);
    fechar('overlay-garantia');
    renderizarGarantias();
  });
  ...
  async function carregarCatalogo(){
    if(db){
      try{
        const snap = await db.collection('cursos').orderBy('criado', 'desc').get();
        catalogo = snap.docs.map(d => Object.assign({}, d.data(), { id: d.id }));
        return;
      }catch(e){ /* cai pro modo local abaixo se o Firestore falhar (ex: sem permissão, offline) */ }
    }
  ...
  async function publicarNaComunidade(p){
    ...
    let ok = false;
    if(db){
      try{ await db.collection('cursos').doc(p.id).set(registro, { merge:true }); ok = true; }
      catch(e){ ok = false; }
    }
    if(!ok){
      if(jaTem) Object.assign(jaTem, registro); else catalogo.push(registro);
      ok = await Guardar.gravar(CHAVE_CATALOGO, catalogo, true);
    }
    p.publicado = true;
    await Guardar.gravar(CHAVE_PRODUTOS, produtos);
    renderizarDashboard();
  ...
  async function carregarAvaliacoes(){
    if(db){
      try{
        ...
        avaliacoes = snap.docs.map(d => Object.assign({}, d.data(), { id: d.id }));
        return;
      }catch(e){ /* cai pro modo local abaixo */ }
    }
  ...
  async function renderizarComunidade(){
    ...
    $('aviso-comunidade').textContent = db
      ? t('aviso_com_publico')
      : t('aviso_com_local');
  ...
  function abrirCurso(id){
    ...
    if(!meu && !inscrito){
      $('btn-matricular').addEventListener('click', async () => {
        ...
        if(db){
          try{ await db.collection('cursos').doc(c.id).update({ alunos: firebase.firestore.FieldValue.increment(1) }); }catch(e){}
        } else {
          await carregarCatalogo();
      ...
      $('lista-notas').querySelectorAll('[data-nota]').forEach(b => b.addEventListener('click', () => {
        ...
        if(!$('btn-enviar-avaliacao')){
          ...
          $('btn-enviar-avaliacao').addEventListener('click', async () => {
            ...
            const registroAvaliacao = { cursoId:c.id, autor:perfil.nome, nota:notaEscolhida, comentario:$('comentario-avaliacao').value.trim(), criado:Date.now() };
            if(db){
              try{ await db.collection('avaliacoes').add(registroAvaliacao); }catch(e){}
            } else {
              avaliacoes.push(Object.assign({ id:idNovo() }, registroAvaliacao));
    ...
    if(btnPerg){
      btnPerg.addEventListener('click', async () => {
        ...
        if(!texto) return;
        const novaPergunta = { autor:perfil.nome, texto, criado:Date.now() };
        if(db){
          try{ await db.collection('cursos').doc(c.id).update({ perguntas: firebase.firestore.FieldValue.arrayUnion(novaPergunta) }); }catch(e){}
        } else {
          await carregarCatalogo();
          const alvo = catalogo.find(x=>x.id===c.id);
          if(alvo){
            alvo.perguntas = alvo.perguntas || [];
            alvo.perguntas.push(novaPergunta);
  ...
  function renderizarContaModal(){
    ...
    if(user){
      const codigoIndicacao = encodeURIComponent((perfil.nome || user.email.split('@')[0]).toLowerCase().replace(/\s+/g,'-'));
      const linkIndicacao = location.origin + location.pathname + '?ref=' + codigoIndicacao;
      area.innerHTML = '<div class="subtitulo-modal">'+esc(t('logado_como'))+' '+esc(user.email||'')+'</div>'
        ...
        + '<div class="bloco" style="margin:18px 0 0;padding:16px">'
          + '<div class="titulo-bloco" style="margin-bottom:8px"><h3 style="font-size:.95rem">'+esc(t('h3_convidar_amigos'))+'</h3></div>'
          + '<div class="aviso" style="margin-bottom:10px">'+esc(t('aviso_convidar_amigos'))+'</div>'
          + '<div class="item-resumo" style="margin-bottom:10px;word-break:break-all;font-size:.78rem">'+esc(linkIndicacao)+'</div>'
          + '<button class="botao-secundario botao-largo" id="btn-compartilhar-indicacao">'+esc(t('btn_compartilhar_link'))+'</button>'
        + '</div>'
      ...
      $('btn-compartilhar-indicacao').addEventListener('click', () => {
        compartilharTexto(t('h3_convidar_amigos'), t('txt_convite_mensagem').replace('{link}', linkIndicacao));
      });
  ...
  function aplicarGateLogin(){
    const bloquear = precisaLogin();
    const xBtn = document.querySelector('#overlay-perfil .fechar-modal');
    const aviso = $('aviso-login-obrigatorio');
    if(xBtn) xBtn.style.display = bloquear ? 'none' : '';
    if(aviso) aviso.style.display = bloquear ? 'block' : 'none';
    if(bloquear) abrir('overlay-perfil');
  }
  renderizarContaModal();
  aplicarGateLogin();
  ...
  if(auth){
    auth.onAuthStateChanged(async (user) => {
      ...
      aplicarGateLogin();
      if(user) fechar('overlay-perfil');
    });
  } else {
    // Firebase indisponível (offline, bloqueado etc.) — não trava a pessoa sem ter como logar.
    fechar('overlay-perfil');
  }
  ...
  if(Motor){
    ...
    rec.addEventListener('result', ev => {
      ...
      campo.value = t;
      if(campo.classList.contains('campo-auto-crescer')){ campo.style.height = 'auto'; campo.style.height = campo.scrollHeight + 'px'; }
    });
  ...
  $('btn-comparar-foto').addEventListener('click', () => $('input-camera-comparar').click());
  $('input-camera-comparar').addEventListener('change', async ev => {
    const arq = ev.target.files[0];
    ev.target.value = '';
    if(!arq) return;
    await tratarArquivo(arq);
    if(anexoPendente){
      $('input-barra').value = t('prompt_comparar_foto');
      enviar();
    }
  });

  (async function iniciar(){
    const [c, conv, p, l, pf, mt, idi, ap, mts, est, des, alp, lem, gar] = await Promise.all([
      ...
      Guardar.ler(CHAVE_ALERTAS_PRECO), Guardar.ler(CHAVE_LEMBRETES), Guardar.ler(CHAVE_GARANTIAS)
    ]);
    ...
    garantias = Array.isArray(gar) ? gar : [];
    ...
    // roda em segundo plano, sem travar a abertura do app
    setTimeout(() => { verificarAlertasVencidosAutomaticamente(); }, 1500);
  })();
})();
